...
# (Use the dataset you previously provided; base volume = 0.320)
# Columns: [pH, temp, time, volume_base (0.320), yield_g]
# -------------------------
DATA = np.array([
...
def train_and_save_model(epochs=250):
    print("Training model (this runs only if model or scaler are missing)...")
...
# -------------------------
# Safe load (compile=False) to avoid 'keras.metrics' deserialization errors
# -------------------------
def load_safe_model():
    if not Path(MODEL_FILE).exists() or not Path(SCALER_FILE).exists():
        train_and_save_model()
    # compile=False avoids loading training config/metrics that cause error
...
# -------------------------
# Prediction logic
# - We feed the model features with base volume (0.320) to match training
# - Then scale result linearly by (requested_volume / base_volume)
# -------------------------
...
<style>
  ...
  *{box-sizing:border-box;font-family:'Poppins',sans-serif}
  body{
    margin:0;
    min-height:100vh;
    background: radial-gradient(circle at 10% 10%, #08304a 0%, var(--bg1) 20%, var(--bg2) 60%);
    color:var(--muted);
    display:flex;
    align-items:center;
  ...
  .card {
    ...
    box-shadow: 0 8px 40px rgba(4,12,20,0.6), inset 0 1px 0 rgba(255,255,255,0.02);
  ...
  .logo {
    ...
    display:flex;align-items:center;justify-content:center;font-weight:700;color:#002;
  ...
  input, select{
    ...
    background:var(--glass-strong);color:#001; font-size:14px; outline:none;
  }
  ...
  .btn {
    margin-top:10px;padding:12px;border-radius:12px;border:none;font-weight:600;
    background:linear-gradient(90deg,var(--accent),#0077ff); color:#012;
    cursor:pointer; box-shadow:0 10px 30px rgba(0,120,255,0.12);
  }

  /* right panel: result card */
  .right {
    padding:18px;border-radius:16px;background: rgba(255,255,255,0.03);backdrop-filter: blur(8px);
    box-shadow: inset 0 1px 0 rgba(255,255,255,0.02);
    display:flex;flex-direction:column;gap:12px;align-items:center;justify-content:center;
  }
  .result-title{font-size:14px;color:var(--muted);opacity:0.9}
  .result-box{
    margin-top:6px;padding:18px;border-radius:12px;background:linear-gradient(180deg, rgba(0,0,0,0.35), rgba(0,0,0,0.45));width:100%;
    text-align:center;color:white;font-weight:700;font-size:20px;box-shadow:0 8px 30px rgba(0,0,0,0.6);
  }
  ...
  <div class="card">
    <div class="left">
      ...
      <form method="POST" action="/">
        ...
        <input name="time" type="number" step="1" value="{{ time or '' }}" required placeholder="e.g. 20" />
